...
def make_synthetic(n_obs, n_features, n_modes, skew_factor, rng):
    """Return DataFrame X and target y."""
    data = np.zeros((n_obs, n_features))
    # --- Multimodal component
    means = np.linspace(0, n_modes*3, n_modes)
...
def sensitivity_grid(runs = 10):
    ...
    for seed in range(runs):
        ...
        for i, skew in enumerate(skew_grid):
            for j, n_modes in enumerate(modes_grid):
                X, y = make_synthetic(5000, 3, n_modes, skew, rng)
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(delta, cmap='RdYlGn', origin='lower',
                   extent=[min(modes_grid), max(modes_grid),
                           min(skew_grid), max(skew_grid)],
                   aspect='auto', 
                   vmin=-abs_max, 
                   vmax=abs_max)
    ax.set_xlabel('# Modes'); ax.set_ylabel('Skew factor')
    ax.set_title('Relative gain of K‑means (1 init, quantile) over Quantile')
    cbar = plt.colorbar(im, ax=ax); cbar.set_label('Relative Error Reduction (%)')
    plt.show()
